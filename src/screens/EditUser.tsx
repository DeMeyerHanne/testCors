...
import { get, putData } from "../utils/dataAccess";
import EditUserPut from "../utils/editUser";

import { useParams } from "react-router-dom";


const AddUser = ({ label, props }: any) => {
	//@ts-ignore
	const { id } = useParams();
	...
	/* Edit user */
	/* `http://smapi-ip-2.azurewebsites.net/api/users` */
	const [editUser, setEditUser] = useState<EditUserPut>({
		userID: "",
		firstName: "",
		lastName: "",
		weight: 0,
	...
	const EditExistingUser = () => {		
		putData("http://smapi-ip-2.azurewebsites.net/api/users", {
			userID: editUser.userID,
			firstName: editUser.firstName,
			lastName: editUser.lastName,
			weight: editUser.weight,
			length: editUser.length,
	...
	return (
		<div className="c-user-layout">
			<div className="c-user-layout__form">
				<h1 className="c-user-layout__title">Edit user</h1>
				<Formik
					initialValues={{
						firstName: '',
						lastName: '',
					...
					onSubmit={values => {
						console.log(values);
						setEditUser(values as EditUserPut);
						EditExistingUser();
					}}
				>
